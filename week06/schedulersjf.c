...
ProcessData find_next_process()
{
    ...
    for (int i = 0; i < data_size; i++)
    {
        ...
        {
            if (data[i].burst < shortestBurstTime)
            {
                shortestBurstTime = data[i].burst;
    ...
    if (shortestProcessIdx >= 0)
    {
        return data[shortestProcessIdx];
    }
    else
    {
        // No process is available to run; return a placeholder value.
...
void schedule_handler(int signum)
{
    ...
    if (next_process.idx != running_process && data[running_process].burst == 0)
    {
        ...
        // if (running_process >= 0)
        // {
        //     // Print a message and stop the current running process.
        ...
        // 3.B. Set next_process as the running process.
        running_process = next_process.idx;

        // 3.C.1. Create a new process for the running process.
        ...
        // else
        // {
        //     // Print a message and resume the process.
